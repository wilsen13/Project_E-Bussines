...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ isRemote: {{ old('is_remote', $job->is_remote) ? 'true' : 'false' }} }">
    ...
    <form action="{{ route('jobs.update', $job->jobID) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $job->title) }}" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors" placeholder="Contoh: Desain Logo Kedai Kopi">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5" required class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors" placeholder="Jelaskan secara detail apa yang harus dilakukan pekerja...">{{ old('description', $job->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="payAmount" name="payAmount" value="{{ old('payAmount', $job->payAmount) }}" required min="0" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors" placeholder="Contoh: 150000">
                ...
                <div>
                    ...
                    <input type="url" id="image_url" name="image_url" value="{{ old('image_url', $job->image_url) }}" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors" placeholder="https://contoh.com/gambar.jpg">
        ...
        <div class="space-y-4 pt-4 border-t border-gray-100">
            ...
            <div x-show="!isRemote" x-transition class="space-y-4 p-4 border border-gray-200 rounded-xl bg-gray-50">
                ...
                <div>
                    ...
                    <input type="text" id="addressLine" name="addressLine" value="{{ old('addressLine', $job->location->addressLine ?? '') }}" :required="!isRemote" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="city" name="city" value="{{ old('city', $job->location->city ?? '') }}" :required="!isRemote" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors">
                        @error('city') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="province" class="block text-sm font-semibold text-gray-700 mb-1">Provinsi</label>
                        <input type="text" id="province" name="province" value="{{ old('province', $job->location->province ?? '') }}" :required="!isRemote" class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 transition-colors">
        ...
        <div class="pt-6 border-t border-gray-100 flex justify-end gap-4">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl shadow-md transition-all">Simpan Perubahan</button>
        </div>
    </form>
</div>
@endsection
